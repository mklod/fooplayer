...
void main() {
  group('LibraryModel.selectTrackClick', () {
    test(
      'plain click selects only the clicked row, replacing any existing selection',
      () {
    ...
    test(
      'ctrl+click toggles a row in/out of the selection and moves the anchor',
      () {
        final m = fixtureLibrary();
        final order = m.visibleTracks;
        m.selectTrackClick(
    ...
    test('shift+click selects the contiguous range from the anchor to the '
    ...
    test(
      'shift+click range works when the anchor sits after the clicked row too',
      () {
        final m = fixtureLibrary();
        final order = m.visibleTracks;
        m.selectTrackClick(
    ...
    test('ctrl+shift+click adds the anchor->clicked range to the existing '
  ...
  group('LibraryModel.selectTrack (single-select, used by double-click)', () {
    test(
      'selects exactly one row, discarding a wider selection, and resets the anchor',
      () {
        final m = fixtureLibrary();
        final order = m.visibleTracks;
        m.selectTrackClick(
  ...
  group(
    'selection clears on the existing filter/search/playlist/folder cascade points',
    () {
      ...
      test('setPlaylist clears the selection', () {
        final m = fixtureLibrary();
        m.playlists = [
          const ManifestPlaylist(name: 'mix', trackIds: ['t1']),
        ];
        m.selectTrackClick(
          't1',
          ctrl: false,
      ...
      test(
        'setSort does NOT clear the selection -- the same rows stay visible, just reordered',
        () {
          final m = fixtureLibrary();
          m.selectTrackClick(
            't1',
