...
void main() {
  ...
  group('readConfigFile', () {
    ...
    test('valid config round-trips as-is', () {
      final f = cfgFile()
        ..writeAsStringSync(jsonEncode({'libraryRoots': ['L:\\music']}));
      expect(readConfigFile(f), {'libraryRoots': ['L:\\music']});
    });

    test('corrupt (invalid JSON) config is backed up to .bad and an empty map is returned', () {
    ...
    test('valid JSON that is not an object is treated as corrupt (backed up, empty map)', () {
      final f = cfgFile()..writeAsStringSync(jsonEncode(['not', 'an', 'object']));
      final result = readConfigFile(f);
      expect(result, isEmpty);
  ...
  group('migrateConfig', () {
    ...
    test('migrates the old singular libraryRoot key to a one-element libraryRoots list', () {
      final config = migrateConfig(
        {'libraryRoot': r'D:\music', 'ui': {'sidebarWidth': 240}},
    ...
    test('an already-v2 config with libraryRoots passes through unchanged', () {
      final config = migrateConfig(
        {'libraryRoots': [r'D:\music', r'E:\more music'], 'ui': {'filterHeight': 200}},
        defaultRoot: defaultRoot,
      );
    ...
    test('an empty libraryRoots list is treated like no roots configured and defaults', () {
      final config = migrateConfig({'libraryRoots': []}, defaultRoot: defaultRoot);
      expect(config.libraryRoots, [defaultRoot]);
    });
  ...
  group('needsMigrationWrite', () {
    ...
    test('true when the old libraryRoot key is still present alongside libraryRoots', () {
      expect(
          needsMigrationWrite(
              {'libraryRoots': [r'D:\music'], 'libraryRoot': r'D:\music'}),
          isTrue);
    });

    test('false for an already-migrated v2 config', () {
      expect(needsMigrationWrite({'libraryRoots': [r'D:\music']}), isFalse);
    });
  });
}
