...
void main() {
  ...
  testWidgets('folder selection cascades into artist panel and track list', (
    ...
    await tester.tap(
      find.text('RockFolder'),
    ); // select folder (basename display)
    await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    'track list shows the union of both artists\' tracks',
    (tester) async {
      final lib = fixtureLibrary();
      final player = PlayerService();
      await tester.pumpWidget(
      ...
      expect(
        find.text('Oldest Song'),
        findsNothing,
      ); // Feed Me's track, filtered out
  ...
  testWidgets(
    'now-playing bar hidden with no track, transport icons exist otherwise',
    (tester) async {
  ...
  testWidgets(
    'AlbumArt caches its future across rebuilds, only re-fetching on contentId change',
    (tester) async {
      var callCount = 0;
    ...
    (tester) async {
      ...
      final seekY = tester
          .getCenter(
            find.descendant(
              of: clusterFinder,
              matching: find.byType(Slider).first,
            ),
          )
          .dy;
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.drag(
        find.byKey(const Key('sidebar-divider')),
        const Offset(80, 0),
      );
      await tester.pumpAndSettle();
