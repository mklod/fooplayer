...
void main() {
  ...
  test('applyFilters ANDs genre/artist and search', () {
    ...
    expect(
      applyFilters(lib, search: 'mus').length,
      3,
    ); // case-insensitive artist match
    expect(
      applyFilters(lib, genre: 'Rock', search: 'beta').single.contentId,
      'b',
    );
  });

  test(
    'null/empty filter matches all; genre filter excludes empty-genre tracks',
    () {
      expect(applyFilters(lib).length, 4);
      expect(
        applyFilters(lib, genre: 'Rock').map((t) => t.contentId),
        isNot(contains('d')),
      );
    },
  );
  ...
  test(
    'applyFilters rootPath matches Track.rootPath exactly (folder filter)',
    () {
      final withRoots = [
  ...
  test(
    'applyFilters artist/album sets OR within the panel, case-insensitively',
    () {
      ...
      expect(
        applyFilters(
          lib,
          artist: {'Muse', 'Feed Me'},
        ).map((t) => t.contentId).toSet(),
        {'a', 'b', 'c', 'd'},
      ); // Muse/muse (a,b,d) + Feed Me (c)
      ...
      expect(
        applyFilters(lib, artist: {'MUSE'}).map((t) => t.contentId).toSet(),
        {'a', 'b', 'd'},
      );
  ...
  test('regression (adversarial review, LOW): trackInFolderScope matches the '
    ...
    expect(
      trackInFolderScope(
        _trackAt('Rock/x.mp3', rootPath: r'L:\OtherMusic'),
        scope,
      ),
      isFalse,
    );
