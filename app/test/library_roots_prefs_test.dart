...
void main() {
  group('LibraryRootsPrefs.addRoot', () {
    test(
      'adds a new root: writer invoked with the updated list, listeners notified',
      () {
    ...
    test(
      'adding an already-configured path no-ops: no write, no notify, list unchanged',
      () {
        final writes = <List<String>>[];
        final prefs = LibraryRootsPrefs(
          roots: ['L:\\music'],
  ...
  group('LibraryRootsPrefs.removeRoot', () {
    test(
      'removing a path that is not configured no-ops: no write, no notify, list unchanged',
      () {
        final writes = <List<String>>[];
        final prefs = LibraryRootsPrefs(
          roots: ['L:\\music'],
    ...
    test(
      'removing a configured path removes it, writes the updated list, and notifies',
      () {
        final writes = <List<String>>[];
        final prefs = LibraryRootsPrefs(
          roots: ['L:\\music', 'L:\\downloads'],
          writer: writes.add,
        );
        var notifications = 0;
        prefs.addListener(() => notifications++);
