...
void main() {
  ...
  test(
      'merges two roots: tracks dedupe by contentId (first root wins) and '
    ...
    await _writeManifest(rootB, tracks: {
      ...
      'shared': _trackJson('B - Shared Song Renamed.mp3', '2099-01-01T00:00:00Z'),
    ...
    expect(shared.relPath, 'A - Shared Song.mp3', reason: 'rootA (listed first) wins the dedupe');
  ...
  test(
      'merges playlists from both roots, suffixing a same-named playlist '
    ...
    await _writeManifest(rootB, tracks: {
      'b1': _trackJson('b1.mp3', '2024-01-02T00:00:00Z'),
    }, playlists: [
      {'name': 'mix', 'track_ids': ['b1']}, // collides with rootA's "mix"
  ...
  test(
      'a root with no .library.json yet is skipped (not fatal) and reported '
    ...
    await _writeManifest(withManifest, tracks: {
  ...
  test(
    ...
    expect(model.rootsMissingManifest, unorderedEquals([rootA.path, rootB.path]));
