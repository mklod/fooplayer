...
| Android app | ✅ Phone-native UI v1 + background audio + one-level Back + tap-to-full-screen. Storage access sorted (all-files, needed because the manifest/artwork sidecars are not media files) and a **Set up** button seeds a root that has no manifest — on a phone there is no CLI, so "seed with foolib" was a dead end. Running live on the Galaxy Tab S9+ off a single `/storage/emulated/0/Music` root, 467 tracks, real download dates. ⚠️ Still the phone layout on a 2800px tablet — responsive large-screen layout is queued. Installed and exercised on a real Galaxy Tab S9+ (SM-X810, Android 16), verified live on the emulator: drawer shell (Library/Folders/Artists/Albums/Playlists/Settings), date feed, mini-player, Now Playing with metro transport (no volume slider — hardware keys own it), browse views, playlist management. Launcher icon + "fooplayer" label |
...
| Test suite | ✅ 914 app tests + 44 core tests; `flutter analyze` at 2 pre-existing style hints in test files; Windows release and debug APK both build from one tree |
