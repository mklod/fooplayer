...
### Task 6: sortable columns (user request 2026-07-24)
Files: `tags.dart` (TrackTags gains `int? durationMs`, captured from the parser metadata during readTags; json round-trip); `track.dart` (+`int? durationMs`), `meta_cache.dart` (apply duration; entries lacking the field = cache miss so duration backfills); `library_model.dart` (+`SortColumn {title, artist, album, duration, dateAdded}` + `bool sortAscending`; default dateAdded desc; `setSort(column)` toggles asc/desc on repeat-click; visibleTracks applies it except playlist mode); `track_list.dart` (header row styled per tokens: 11px w600 uppercase labels Title/Artist/Album/Time/Date with sort arrow on active column, click → setSort; row layout becomes columns aligned to the header: title+artist flexible, album flexible, duration right 44px `m:ss`, date 82px). Tests: sort logic unit tests (each column asc/desc, playlist order unaffected); header widget test (click Artist → model.sortColumn artist asc, click again → desc, arrow flips).

### Task 7: scoped 5-root reseed (controller-approved data op; user directive verbatim: "use L:\music (original structure) — but select folders ONLY: monthly, loose tracks old, loose track 2020, alt times, albums")
Controller-run: (1) split `L:\music (original structure)\.hash_cache.json` into per-folder caches (strip path prefix) written into each of: `monthly`, `loose tracks - old`, `loose tracks - 2020 and later`, `alternative times`, `albums`; (2) `foolib seed --root <folder> --metadb-json core/tools/metadb_dates.json --apply` per folder (for `alternative times` add `--playlists "L:\music (original structure)"` so the mike playlist imports there); (3) set app config `libraryRoots` to exactly those five absolute paths. The old root-level `.library.json` stays untouched (harmless; the CLI can still use it).

### Task 8 (controller): rebuild, relaunch, screenshot set, ledger, then whole-branch final review.
